...
export function calculateTotalPrice(orderData = {}) {
    ...
    return {
        ...
        backendPricing: {
            ...
            financialBreakdown: {
                deliveryTotal: Math.round(deliveryTotal),      // What you should receive
                customerAmount: paystackCalc.finalCustomerAmount, // What customer pays
                processingFee: paystackCalc.processingFee,     // Paystack's cut
                netAmount: Math.round(deliveryTotal),          // What you actually get
            ...
            totalAmount: paystackCalc.finalCustomerAmount,
            currency: 'NGN'
        },
...
// Keep your original weight-based logic as separate function
function determineOptimalVehicleByWeight(orderData) {
    const weight = getPackageWeight(orderData);
    const category = orderData.package?.category || 'parcel';

    if (weight <= 5 && ['document', 'medicine'].includes(category)) return 'bicycle';
    if (weight <= 15 && category !== 'furniture') return 'motorcycle';
    if (weight <= 30) return 'tricycle';
    if (weight <= 100) return 'car';
    if (weight <= 500) return 'van';
    return 'truck';
}

// Add vehicle capability check
function canVehicleHandlePackage(vehicleType, orderData) {
    const weight = getPackageWeight(orderData);
    const category = orderData.package?.category || 'parcel';

    const limits = {
        bicycle: { maxWeight: 5, cannotHandle:  ['fragile', 'cake', 'furniture', 'electronics'] },
        motorcycle: { maxWeight: 25, cannotHandle: ['furniture'] },
        tricycle: { maxWeight: 100, cannotHandle: [] },
        car: { maxWeight: 300, cannotHandle: [] },
        van: { maxWeight: 500, cannotHandle: [] },
        truck: { maxWeight: Infinity, cannotHandle: [] }
    };

    const limit = limits[vehicleType];
    return weight <= limit.maxWeight && !limit.cannotHandle.includes(category);
}
