...
class SessionManager {
    ...
    static async logout() {
        try {
            this.isLoggingOut = true; // ✅ Set flag FIRST
            console.log("[SessionManager] 🚪 Initiating logout...");

            // Clear session tokens (preserves role and onboarded)
            await SecureStorage.clearSessionOnly();

            // Clear user data from MMKV
            MMKVStorage.removeItem('userData');

            // Clear order data from MMKV
            ...
            this.isLoggingOut = false; // ✅ Reset on error
