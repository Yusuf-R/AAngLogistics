...
function UpdatePin({userData}) {
    const [step, setStep] = useState(1); // 1: Token, 2: Current PIN, 3: New PIN
    ...
    const [modalMessage, setModalMessage] = useState('Updating PIN...');

    //
    const mutation = useMutation({
        mutationKey: ['UpdatePin'],
        mutationFn: ClientUtils.UpdatePin, // Assuming you have this function
    });
    ...
    const [visibleCurrentPinDigits, setVisibleCurrentPinDigits] = useState(new Set());
    const [visibleNewPinDigits, setVisibleNewPinDigits] = useState(new Set());
    const currentPinMaskTimers = useRef({});
    ...
    const getCurrentInputData = () => {
        switch (step) {
            case 2:
                return {
                    code: currentPinCode,
                    setCode: setCurrentPinCode,
                    visibleDigits: visibleCurrentPinDigits,
                    setVisibleDigits: setVisibleCurrentPinDigits,
                    maskTimers: currentPinMaskTimers,
                    maxLength: 6
                };
            case 3:
                return {
    ...
    const handleContinue = async () => {
        ...
        if (step > 1) {
            const inputData = getCurrentInputData();
            const currentInput = step === 2 ? currentPin : newPin;
        ...
        if (step === 1) {
            ...
            if (currentPin === newPin) {
                ...
                Object.values(newPinMaskTimers.current).forEach(timer => clearTimeout(timer));
                newPinMaskTimers.current = {};
                return;
            }

            setModalStatus('loading');
            setModalMessage('Updating your PIN...');
            ...
            mutation.mutate(payload, {
                onSuccess: async (resp) => {
                    ...
                    setModalMessage('PIN updated successfully! 🚀');
    ...
    useEffect(() => {
        return () => {
            [currentPinMaskTimers, newPinMaskTimers].forEach(timerRef => {
                            ...
                            <TouchableOpacity
                                onPress={() => {
                                    if (step === 2) {
                                        ...
                                        setCurrentPin('');
                                        setCurrentPinCode(['', '', '', '', '', '']);
                                        setVisibleCurrentPinDigits(new Set());
                                    } else if (step === 3) {
                                        setStep(2);
                                        setNewPin('');
                                        setNewPinCode(['', '', '', '', '', '']);
